...
* Base Classes
  Each Entity is represented as class. 
#+NAME: base_classes
#+BEGIN_SRC python
...
* Type Predicates
  ...
  The =Type Predicates= checks the type of the given input(s).
...
* Question
  A =Question= entity has fields =name= and =question_type= of type string.
...
* Feedback
  ...
  string, =exp_name= of type string and responses of type list of =Responses=
  or an empty list.
